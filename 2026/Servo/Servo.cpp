...
void Servo_Driver::init(TIM_HandleTypeDef *htim, uint8_t channel){
...
void Servo_Driver::zero_in(){
	if (!_htim) return;
	uint32_t ccr = (uint32_t)zero_pulse;
...
float Servo_Driver::angle_to_duty(){
    ...
    if (pwm < (float)min_pulse) pwm = (float)min_pulse;
    if (pwm > (float)max_pulse) pwm = (float)max_pulse;

    return pwm;
}

void Servo_Driver::setAngle(int32_t new_angle)
{
    if (_htim == nullptr) return;

    // clamp to allowed range
    if (new_angle < (int32_t)min_angle) new_angle = (int32_t)min_angle;
    if (new_angle > (int32_t)max_angle) new_angle = (int32_t)max_angle;
...
void Servo_Driver::set_servo(){
	if (!_htim) return;
    uint32_t ccr = (uint32_t)angle_to_duty();
    __HAL_TIM_SET_COMPARE(_htim, _channel, ccr);
}
