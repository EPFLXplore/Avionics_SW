...
pushd firmware/mcu_ws > /dev/null
    ...
    pushd extra_packages > /dev/null
        ...
        # Copy avionics msgs from the submodule into the custom_msg package
        CUSTOM_MSG_DIR=/project/ERC_SE_CustomMessages
        if [ -d "$CUSTOM_MSG_DIR" ]; then
            mkdir -p custom_msg/msg/avionics
            cp $CUSTOM_MSG_DIR/msg/avionics/* custom_msg/msg/avionics/
            echo "Copied avionics msgs from ERC_SE_CustomMessages submodule."
        else
            echo "WARNING: ERC_SE_CustomMessages submodule not found at $CUSTOM_MSG_DIR"
        fi
    popd > /dev/null
